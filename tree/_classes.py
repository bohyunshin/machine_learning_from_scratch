import numpy as np
from _criterion import gini, entropy, mse, mae
...
class Leaf:
    ...
    def __init__(self, label, idx, class_name):
        if class_name == 'DecisionTreeClassifier_':
            self.predictions = count(label, idx)
        else:
            self.predictions = np.mean(label[idx])
...
class BaseDecisionTree:
    ...
    def information_gain(self, y, left_idx, right_idx, impurity):
        ...
        left_impurity = CRITERIA[self.criterion](y, left_idx).impurity
        right_impurity = CRITERIA[self.criterion](y, right_idx).impurity
        info_gain = impurity - p*left_impurity - (1-p)*right_impurity
        return info_gain
    ...
    def find_best_split(self, X, y, y_idx, X_conti_cols):
        ...
        impurity = CRITERIA[self.criterion](y, y_idx).impurity
    ...
    def build_tree(self, X, y, y_idx, X_conti_cols):
        ...
        if best_gain == 0:
            return Leaf(y, y_idx, self.__class__.__name__)
...
class DecisionTreeClassifier_(BaseDecisionTree):
    """
    Child class for classification tree.
    This class inherits from BaseDecisionTree class
    """
    def __init__(self,
                 criterion,
                 num_vars,
    ...
    def predict(self, test_data, tree, X_conti_cols):
        ...
        if feature_name in X_conti_cols:
            if test_data[feature_name] <= feature_value:
                return self.predict(test_data, tree.true_branch, X_conti_cols)

            else:
                return self.predict(test_data, tree.false_branch, X_conti_cols)
...
class DecisionTreeRegressor_(BaseDecisionTree):
    ...
    def predict(self, test_data, tree, X_conti_cols):
        ...
        # Check if we are at a leaf node
        if isinstance(tree, Leaf):
            return np.mean(tree.predictions)
